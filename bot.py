...
async def on_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Greet new members via ChatMemberHandler."""
...
async def on_new_member_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Backup: catch new_chat_members service messages."""
...
async def send_welcome(context: ContextTypes.DEFAULT_TYPE, user):
...
async def handle_decision(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        try:
            ...
            await context.bot.send_message(
                chat_id=ADMIN_USER_ID,
                text=f"⚠️ Approved but couldn't DM @{username} — DMs may be disabled.\nError: {e}"
            )

        # Remove from gateway group
        try:
        ...
        await query.edit_message_text(f"✅ @{username} has been *approved*, sent the invite link, and removed from The GateWay!.", parse_mode="Markdown")
        ...
        try:
            ...
            await context.bot.send_message(
                chat_id=ADMIN_USER_ID,
                text=f"⚠️ Denied but couldn't DM @{username} — DMs may be disabled.\nError: {e}"
            )
        ...
        await query.edit_message_text(f"❌ @{username} has been *denied* and removed from The GateWay!.", parse_mode="Markdown")
...
def main():
    ...
    app.add_handler(MessageHandler(
        filters.Chat(GATEWAY_GROUP_ID) & filters.StatusUpdate.NEW_CHAT_MEMBERS,
        on_new_member_message
    ))
    app.add_handler(MessageHandler(
