...
MAIN_GROUP_INVITE = "https://t.me/+SobEgcQjIE83NzA5"
...
DM_REMINDER = (
    "📩 *One more thing!*\n\n"
    "Before you start submitting, tap the link below and press *Start* so we can send your decision privately:\n"
    "👉 @TheGateWayKeeperBot"
)
...
async def on_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        # Track EVERY message from this user for cleanup
        if user.id in pending_submissions:
            if update.message.message_id not in pending_submissions[user.id]["message_ids"]:
                pending_submissions[user.id]["message_ids"].append(update.message.message_id)
        elif user.id in submitted_users and user.id in submitted_message_ids:
            if update.message.message_id not in submitted_message_ids[user.id]["message_ids"]:
                submitted_message_ids[user.id]["message_ids"].append(update.message.message_id)
